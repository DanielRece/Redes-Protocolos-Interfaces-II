...
static void coap_example_server(void *p)
{
    ...
    while (1) {
        ...
        resource = coap_resource_init(coap_make_str_const("time"), resource_flags);
        coap_register_request_handler(resource, COAP_REQUEST_GET, hnd_get_fetch_time);
        coap_register_request_handler(resource, COAP_REQUEST_FETCH, hnd_get_fetch_time);
        coap_register_request_handler(resource, COAP_REQUEST_PUT, hnd_put_time);
        coap_register_request_handler(resource, COAP_REQUEST_DELETE, hnd_delete_time);
        coap_resource_set_get_observable(resource, 1);

        coap_add_attr(resource, coap_make_str_const("ct"), coap_make_str_const("0"), 0);
        coap_add_attr(resource, coap_make_str_const("title"), coap_make_str_const("\"Internal Clock\""), 0);
        coap_add_attr(resource, coap_make_str_const("rt"), coap_make_str_const("\"ticks\""), 0);
        coap_add_attr(resource, coap_make_str_const("if"), coap_make_str_const("\"clock\""), 0);
