...
hnd_get_temperature(coap_resource_t *resource,
  ...
  size_t len;
  int temperature;
  coap_str_const_t *celsius = coap_make_str_const("celsius");
  coap_str_const_t *fahrenheit = coap_make_str_const("fahrenheit");

  /* Generar un valor de temperatura aleatorio (puedes reemplazar esto con tu lógica real) */
  temperature = rand() % 101; // Valor aleatorio entre 0 y 100

  /* Obtener la temperatura en grados Celsius por defecto */
  if (query != NULL && coap_string_equal(query, fahrenheit)) {
    /* Si la consulta es 'fahrenheit', convertir a grados Fahrenheit */
    temperature = (temperature * 9 / 5) + 32;
  }

  /* Formatear la temperatura como una cadena */
  len = snprintf((char *)buf, sizeof(buf), "Temperature: %d degrees %s", temperature, query ? (const char *)query->s : "celsius");

  /* Establecer la respuesta como contenido */
  coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);

  /* Agregar la respuesta como un bloque de datos grande */
...
init_resources(coap_context_t *ctx) {
  ...
  coap_register_request_handler(r, COAP_REQUEST_GET, hnd_get_temperature);
